...
async function main() {
  const from = "0x6bd5d6fE42419e9039323f9D25B6484F5344f00D";
  const airdropAmount = 10 * 1e18;

  const unsentAddrs = await getUnsentAddrs().slice(0, 50);
  unsentAddrs.forEach(addr =>{
    lrcToken.transferFrom(from,
                          addr,
                          airdropAmount,
                          {
                            from: fromAddr,
